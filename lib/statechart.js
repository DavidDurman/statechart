...
(function(root, factory){
    ...
    var Statechart = {
        ...
        construct: function(initialState){
            ...
            this.pushStates = this.pushStates || {};
        },
        ...
        /**
         * add the given state to the pushdown stack
         *
         * @param target state
         */
        pushState: function(target) {
            assert(this.pushStates[target]);
            var state = this.pushStates[target];
            if (state.entry) {
                state.entry.call(this);
            }
            this.pushDownStack.push(state);
        },

        /**
         * remove given state from pushdown from stack
         *
         * @param stateName
         * @returns state object
         * @private
         */
        _removePushdownState: function(stateName) {
            var i, currentState, len;
            assert(stateName);
            for (i = 0, len = this.pushDownStack.length; i < len; i++) {
                currentState = this.pushDownStack[i];
                if (currentState === this.pushStates[stateName]) {
                    this.pushDownStack.splice(i, 1);
                    return currentState;
                }
            }
        ...
        /**
         * remove the given state from pushDownStack
         * and calls exit function if exists
         *
         * @param stateName
         */
        popState: function(stateName) {
            var state = this._removePushdownState(stateName);
            if (state.exit) {
        ...
        /**
         * check if event can be handled by a pushdown state
         *
         * @param anEvent
         * @returns bool false or function pointer
         */
        checkPushStates: function(anEvent) {
            var found = false;
            if (anEvent.type === 'entry' || anEvent.type === 'exit') {
                return found;
            }
            for (var i = this.pushDownStack.length-1; i >= 0; i--) {
                if (this.pushDownStack[i][anEvent.type]) {
                    return this.pushDownStack[i][anEvent.type];
                }
            }
            return found;
        },
        ...
        dispatchEvent: function(anEvent, state, act){
            ...
            act = this.checkPushStates(anEvent) || act || state[anEvent.type];
